...
const Add_Equipment = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${baseURL}/inventory/api/equipment/${pk}/`, {
          headers: {
  ...
  const handleSubmit = (event) => {
    ...
    axios.put(`${baseURL}/inventory/api/equipment/${pk}/`, formData, {
      headers: {
        Authorization: `Token ${token}`,
      },
    })
  ...
  <form className="flex flex-col w-[1120px] px-6 pb-12 font-medium bg-slate-50 max-md:px-5" onSubmit={handleSubmit}>
    <div className="flex flex-col pt-6 pb-12 bg-white max-md:max-w-full">
      ...
      <Link to="/Inventory/Equipment">
        <button className="grow justify-center px-8 py-4 text-blue-700 rounded-lg border border-blue-700 border-solid max-md:px-5" >
          Cancel
        </button></Link>
