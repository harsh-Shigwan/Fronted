...
const AttendanceGrid = () => {
  ...
  const appointmentTimeSlots = [
    {
      time: ["09:00 AM - 10:00 AM", "10:30 AM - 11:30 AM"],
      doctorName: "Dr. Smith",
      date: "2024-07-01"
    },
    {
      time: ["11:00 AM - 12:00 PM", "01:00 PM - 02:00 PM"],
  ...
  return (
    <div className="container w-[1120px]">
      <h1 className="text-2xl font-bold mb-4">Doctor Appointment Schedule</h1>
      ...
      <div className="overflow-x-auto no-scrollbar" ref={dateContainerRef}>
        <table className="min-w-full bg-white">
          <thead className="bg-gray-800 text-white h-[55px] border-indigo-200">
            <tr>
              <th className="sticky left-0 z-10 py-2 px-4 bg-indigo-100 text-text-body-light text-lg font-medium">ID</th>
              <th className="sticky left-[55px] z-10 py-2 px-4 bg-indigo-100 text-text-body-light text-lg font-medium">Doctor Name</th>
                ...
                {dates.map(date => (
                  <td key={date} className="px-4 py-2">
                    {appointmentTimeSlots
                      .filter(slot => slot.date === date && slot.doctorName === st.name) // Filter by date and doctorName
                      .flatMap(slot => slot.time.map((timeSlot, index) => (
